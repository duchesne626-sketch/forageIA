    ...
    <div style={{ minHeight: "100vh", background: "#f0f7f0", fontFamily: "Arial, sans-serif" }}>
      <nav style={{ background: "#2D6A4F", padding: "15px 30px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative" }}>
        ...
        <div className={`nav-buttons ${menuOuvert ? "ouvert" : ""}`}>
          <button onClick={() => { setPage("foret"); setMenuOuvert(false) }} style={navBtn(page === "foret")}>🌲 Forêt</button>
          <button onClick={() => { setPage("frigo"); setMenuOuvert(false) }} style={navBtn(page === "frigo")}>🧊 Frigo</button>
          <button onClick={() => { setPage("recette"); setMenuOuvert(false) }} style={navBtn(page === "recette")}>🍽️ Recette</button>
          <button onClick={() => { setPage("historique"); setMenuOuvert(false) }} style={navBtn(page === "historique")}>📖 Historique</button>
          <button onClick={() => { setPage("carte"); setMenuOuvert(false) }} style={navBtn(page === "carte")}>🗺️ Carte</button>
         <button onClick={() => { setPage("profil"); setMenuOuvert(false) }} style={navBtn(page === "profil")}>👤 Profil</button>
<button onClick={() => { setPage("encyclopedie"); setMenuOuvert(false) }} style={navBtn(page === "encyclopedie")}>📚 Encyclopédie</button>
          <div>
            {utilisateur ? (
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                <span style={{ color: "white", fontSize: "13px" }}>👤 {utilisateur.email}</span>
                <button onClick={() => signOut(auth)} style={navBtn(false)}>Déconnexion</button>
              </div>
            ) : (
              <button onClick={() => { setPage("auth"); setMenuOuvert(false) }} style={navBtn(page === "auth")}>🔐 Connexion</button>
            )}
          </div>
        </div>
      </nav>
    ...
    <div style={{ margin: "-40px -20px 0" }}>
      <div className="hero">
        ...
        <p style={{ color: "rgba(255,255,255,0.85)", fontSize: "18px", marginBottom: "8px", maxWidth: "500px" }}>
          De la nature à l'assiette, zéro gaspillage
        </p>
        <p style={{ color: "rgba(255,255,255,0.6)", fontSize: "13px", marginBottom: "40px" }}>
          Identifie • Cuisine • Savoure
        </p>
      ...
      <div style={{ padding: "50px 30px" }}>
        <p style={{ textAlign: "center", color: "#888", fontSize: "12px", letterSpacing: "3px", textTransform: "uppercase", marginBottom: "12px" }}>Comment ça fonctionne</p>
        <h2 style={{ textAlign: "center", color: "#1b4332", marginBottom: "35px", fontSize: "26px", fontWeight: "700" }}>
          Du terrain à l'assiette en 3 étapes
        </h2>
        <div style={{ display: "flex", gap: "16px", justifyContent: "center", flexWrap: "wrap" }}>
          {[
            { emoji: "🌲", titre: "Identifie", desc: "Photo d'une plante → l'IA l'identifie et vérifie sa comestibilité", page: "foret", couleur: "#1b4332" },
            { emoji: "🧊", titre: "Combine", desc: "Analyse ton frigo et marie tes trouvailles avec tes restes", page: "frigo", couleur: "#2d6a4f" },
            { emoji: "🍽️", titre: "Cuisine", desc: "Génère une recette narrative unique, zéro gaspillage", page: "recette", couleur: "#40916c" },
          ].map((item, i) => (
        ...
        <div className="stats-section">
          <p style={{ textAlign: "center", color: "#888", fontSize: "12px", letterSpacing: "3px", textTransform: "uppercase", marginBottom: "25px" }}>ForageIA en chiffres</p>
          <div style={{ display: "flex", justifyContent: "center", gap: "50px", flexWrap: "wrap" }}>
            {[
              { nb: "15+", label: "Plantes répertoriées" },
              { nb: "3", label: "Modules IA intégrés" },
              { nb: "∞", label: "Recettes possibles" },
              { nb: "0", label: "Gaspillage alimentaire" },
            ].map((stat, i) => (
        ...
        <div style={{ background: "#1b4332", borderRadius: "20px", padding: "35px", marginTop: "20px", textAlign: "center" }}>
          <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "13px", marginBottom: "8px" }}>Prêt à explorer ?</p>
          <h3 style={{ color: "white", fontSize: "22px", marginBottom: "20px" }}>Commence ta première cueillette IA 🌿</h3>
...
function Foret({ plantes, setPlantes }) {
  ...
  return (
    <div>
      <h2 style={{ color: "#2D6A4F" }}>🌲 Identification de plantes</h2>
      <div style={{ background: "white", borderRadius: "16px", padding: "30px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        ...
        {photo && <p style={{ color: "#2D6A4F", fontSize: "13px" }}>📷 Photo sélectionnée : {photo.name}</p>}
        <button onClick={analyser} disabled={!photo || chargement} style={btnPrincipal}>
          {chargement ? "Analyse en cours..." : "🔍 Identifier la plante"}
        </button>
      </div>
      {erreur && (
      ...
      {resultat && (
        <div style={{ background: "white", borderRadius: "16px", padding: "30px", marginTop: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
          ...
          <p><strong>Confiance :</strong> {resultat.niveau_confiance}%</p>
          <p>{resultat.description}</p>
          <p><strong>💡 Conseil :</strong> {resultat.conseil}</p>
          {resultat.sosies_dangereux && <p style={{ color: "#c0392b" }}><strong>⚠️ Sosies dangereux :</strong> {resultat.sosies_dangereux}</p>}
          <p style={{ color: "#888", fontSize: "12px", marginTop: "15px", fontStyle: "italic", borderTop: "1px solid #eee", paddingTop: "10px" }}>
            🤖 Ce résultat est généré par intelligence artificielle et peut contenir des erreurs. Ne consommez jamais une plante sans une identification certaine par un expert.
          </p>
        </div>
      )}
      {plantes.length > 0 && (
        <div style={{ background: "#e8f5e9", borderRadius: "12px", padding: "15px", marginTop: "15px" }}>
          <strong>🌿 Plantes collectées :</strong> {plantes.join(", ")}
        </div>
      )}
    </div>
  )
  ...
  return (
    <div>
      <h2 style={{ color: "#2D6A4F" }}>🧊 Analyse du frigo</h2>
      <div style={{ background: "white", borderRadius: "16px", padding: "30px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        <div style={{ display: "flex", gap: "10px", marginBottom: "15px", flexWrap: "wrap" }}>
          <label style={{ ...btnPrincipal, background: "#52B788", cursor: "pointer", display: "inline-block" }}>
            📁 Choisir une photo
        ...
        {photo && <p style={{ color: "#2D6A4F", fontSize: "13px" }}>📷 Photo sélectionnée : {photo.name}</p>}
        <button onClick={analyser} disabled={!photo || chargement} style={btnPrincipal}>
          {chargement ? "Analyse en cours..." : "🔍 Analyser mon frigo"}
        </button>
      </div>
      {erreur && (
        <div style={{ background: "#ffe0e0", borderRadius: "12px", padding: "15px", marginTop: "15px", color: "#c0392b" }}>
...
function Recette({ plantes, ingredientsFrigo }) {
  ...
  return (
    <div>
      <h2 style={{ color: "#2D6A4F" }}>🍽️ Génération de recette</h2>
      <div style={{ background: "white", borderRadius: "16px", padding: "30px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        {plantes.length === 0 && ingredientsFrigo.length === 0 && (
          <p style={{ color: "#888" }}>💡 Va d'abord identifier des plantes en forêt et analyser ton frigo !</p>
        )}
        {(plantes.length > 0 || ingredientsFrigo.length > 0) && (
          <>
            {plantes.length > 0 && <p><strong>🌿 Plantes :</strong> {plantes.join(", ")}</p>}
            {ingredientsFrigo.length > 0 && <p><strong>🧊 Frigo :</strong> {ingredientsFrigo.join(", ")}</p>}
            <button onClick={generer} disabled={chargement} style={btnPrincipal}>
              {chargement ? "Génération en cours..." : "✨ Générer ma recette"}
            </button>
          </>
        )}
      </div>
      ...
      {resultat && (
        <div style={{ background: "white", borderRadius: "16px", padding: "30px", marginTop: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
          ...
          <h4 style={{ color: "#2D6A4F" }}>Ingrédients</h4>
          <ul>{resultat.ingredients.map((ing, i) => (
            <li key={i}>
              {typeof ing === "object" ? `${ing.quantite || ""} ${ing.unite || ""} ${ing.nom}`.trim() : ing}
            </li>
          ))}</ul>
          <h4 style={{ color: "#2D6A4F" }}>Étapes</h4>
          <ol>{resultat.etapes.map((e, i) => <li key={i} style={{ marginBottom: "8px" }}>{e}</li>)}</ol>
          <p style={{ background: "#e8f5e9", borderRadius: "8px", padding: "12px" }}>💡 <strong>Conseil du chef :</strong> {resultat.conseil_chef}</p>
          <p style={{ color: "#2D6A4F", fontSize: "13px" }}>🌍 {resultat.impact_environnemental}</p>
          <div style={{ display: "flex", gap: "10px", marginTop: "15px", flexWrap: "wrap" }}>
            <button onClick={async () => {
              ...
              alert("✅ Recette sauvegardée dans ton historique !")
    ...
    <div style={{ maxWidth: "400px", margin: "0 auto" }}>
      <div style={{ background: "white", borderRadius: "16px", padding: "40px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ marginBottom: "15px" }}>
          <label style={{ display: "block", marginBottom: "5px", color: "#555" }}>Adresse e-mail</label>
        ...
        <div style={{ marginBottom: "20px" }}>
          <label style={{ display: "block", marginBottom: "5px", color: "#555" }}>Mot de passe</label>
          <input type="password" value={motDePasse} onChange={e => setMotDePasse(e.target.value)} placeholder="minimum 6 caractères"
            style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1px solid #ddd", fontSize: "14px", boxSizing: "border-box" }} />
        </div>
        {erreur && (
...
function Historique({ utilisateur }) {
  ...
  const supprimer = async (id, e) => {
    e.stopPropagation()
    if (window.confirm("Tu veux vraiment supprimer cette recette ?")) {
  ...
  return (
    <div>
      <h2 style={{ color: "#2D6A4F" }}>📖 Mes recettes sauvegardées</h2>
      ...
      {!chargement && recettes.length === 0 && (
        <div style={{ background: "white", borderRadius: "16px", padding: "30px", textAlign: "center", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
          <p style={{ color: "#888" }}>📭 Aucune recette sauvegardée pour l'instant.</p>
          <p style={{ color: "#888", fontSize: "14px" }}>Génère ta première recette dans l'onglet Recette !</p>
        </div>
      )}
        ...
        <div key={recette.id} style={{ background: "white", borderRadius: "16px", padding: "25px", marginBottom: "15px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", cursor: "pointer" }}
          ...
          {recetteOuverte === recette.id && (
            <div style={{ marginTop: "20px", borderTop: "1px solid #eee", paddingTop: "20px" }}>
              ...
              <p style={{ background: "#e8f5e9", borderRadius: "8px", padding: "12px" }}>💡 <strong>Conseil du chef :</strong> {recette.conseil_chef}</p>
...
function Carte({ plantes }) {
  ...
  const calendrierSaison = [
    { nom: "Pissenlit", mois: [4, 5, 6], emoji: "🌼", description: "Feuilles et fleurs comestibles" },
    { nom: "Ail des ours", mois: [4, 5], emoji: "🧄", description: "Feuilles très parfumées" },
    { nom: "Framboisier sauvage", mois: [7, 8], emoji: "🫐", description: "Fruits juteux et sucrés" },
    { nom: "Bleuet sauvage", mois: [7, 8, 9], emoji: "🫐", description: "Abondant dans les tourbières" },
    { nom: "Chanterelle", mois: [7, 8, 9], emoji: "🍄", description: "Champignon doré très savoureux" },
    { nom: "Cèpe de Bordeaux", mois: [8, 9, 10], emoji: "🍄", description: "Champignon noble des forêts" },
    { nom: "Morille", mois: [4, 5], emoji: "🍄", description: "Champignon printanier rare" },
    { nom: "Salicaire", mois: [6, 7, 8], emoji: "🌸", description: "Fleurs et feuilles comestibles" },
    { nom: "Chicouté", mois: [7, 8], emoji: "🟡", description: "Fruit nordique très rare" },
    { nom: "Sureau du Canada", mois: [8, 9], emoji: "🫐", description: "Baies pour sirops et confitures" },
    { nom: "Épilobe", mois: [6, 7, 8], emoji: "🌺", description: "Jeunes pousses comestibles" },
    { nom: "Pleurote", mois: [9, 10, 11], emoji: "🍄", description: "Sur les troncs d'arbres morts" },
  ]
    ...
    <div>
      ...
      {position && (
        <>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "15px" }}>
                ...
                <div style={{ display: "flex", gap: "20px" }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <p style={{ margin: 0, color: "#888", fontSize: "11px" }}>Humidité</p>
                  </div>
                  <div style={{ textAlign: "center" }}>
                    <p style={{ margin: 0, fontSize: "20px" }}>💨</p>
                    <p style={{ margin: 0, color: "#888", fontSize: "13px" }}>{meteo.vent} km/h</p>
                    <p style={{ margin: 0, color: "#888", fontSize: "11px" }}>Vent</p>
                  </div>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <p style={{ margin: 0, color: meteo.bon_pour_cueillette ? "#27ae60" : "#e67e22", fontSize: "13px", fontWeight: "bold" }}>
                      {meteo.bon_pour_cueillette ? "Idéal" : "Pas idéal"}
                    </p>
                    <p style={{ margin: 0, color: "#888", fontSize: "11px" }}>Cueillette</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div style={{ background: "#e8f5e9", borderRadius: "12px", padding: "15px", marginBottom: "15px" }}>
            <strong>🌿 Zones probables autour de toi :</strong>
          ...
          <div style={{ borderRadius: "16px", overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
            <MapContainer center={position} zoom={14} style={{ height: "450px", width: "100%" }}>
              ...
              <Marker position={position}><Popup>📍 Tu es ici !</Popup></Marker>
      ...
      {!position && !chargement && (
        <div style={{ background: "white", borderRadius: "16px", padding: "40px", textAlign: "center", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
          ...
          <p style={{ color: "#888" }}>Entre ta ville ou clique sur "Me localiser" pour voir les zones de cueillette !</p>
        </div>
      )}
      ...
      <div style={{ background: "white", borderRadius: "16px", padding: "25px", marginTop: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        <h3 style={{ color: "#2D6A4F", marginTop: 0 }}>
          📅 Calendrier de cueillette — {new Date().toLocaleString("fr-CA", { month: "long" })}
        </h3>
        <h4 style={{ color: "#27ae60", marginBottom: "10px" }}>✅ En saison maintenant</h4>
        {plantesEnSaison.length === 0 && <p style={{ color: "#888", fontSize: "14px" }}>Aucune plante en saison ce mois-ci.</p>}
        ...
        <h4 style={{ color: "#e67e22", marginBottom: "10px" }}>⏳ Hors saison</h4>
          ...
          {plantesHorsSaison.map((plante, i) => (
            <div key={i} style={{ background: "#f5f5f5", borderRadius: "12px", padding: "10px 14px", minWidth: "130px", opacity: 0.7 }}>
              ...
              <div style={{ color: "#aaa", fontSize: "11px" }}>
                Mois : {plante.mois.map(m => new Date(2024, m - 1).toLocaleString("fr-CA", { month: "short" })).join(", ")}
              </div>
            </div>
          ))}
        </div>
...
function Profil({ utilisateur, plantes }) {
  ...
  return (
    <div>
      <h2 style={{ color: "#2D6A4F" }}>👤 Mon profil</h2>
      <div style={{ background: "white", borderRadius: "16px", padding: "30px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", marginBottom: "20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "20px", marginBottom: "20px" }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, color: "#888", fontSize: "13px" }}>Membre ForageIA</p>
            <p style={{ margin: "3px 0 0", color: "#888", fontSize: "13px" }}>
              Inscrit le {new Date(utilisateur.metadata.creationTime).toLocaleDateString("fr-CA")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "white", borderRadius: "16px", padding: "25px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        <h3 style={{ color: "#2D6A4F", marginTop: 0 }}>⚙️ Paramètres</h3>
        {motDePasseEnvoye ? (
          <div style={{ background: "#e8f5e9", borderRadius: "8px", padding: "12px", color: "#27ae60" }}>
            ✅ Email de réinitialisation envoyé à {utilisateur.email}
          </div>
        ) : (
...
const rechercherIA = async () => {
  ...
  const plantes = [
    ...
    { nom: "Épilobe", nom_latin: "Epilobium angustifolium", emoji: "🌺", comestible: "oui", saison: [6, 7, 8], parties: "Jeunes pousses, feuilles, fleurs", habitat: "Clairières, coupes forestières, bords de routes", description: "L'épilobe à feuilles étroites est une plante pionnière aux belles fleurs roses. Les jeunes pousses printanières se mangent comme des asperges, et les feuilles peuvent être utilisées en thé.", conseil: "Cueillir les jeunes pousses quand elles mesurent moins de 20 cm pour la meilleure saveur.", danger: null },
    { nom: "Sureau du Canada", nom_latin: "Sambucus canadensis", emoji: "🫐", comestible: "avec prudence", saison: [8, 9], parties: "Fleurs, fruits mûrs cuits", habitat: "Lisières de forêts, zones humides, bords de routes", description: "Le sureau produit de belles grappes de petites baies noires très utilisées pour les sirops, confitures et vins. Ses fleurs blanches sont comestibles en beignets.", conseil: "Les baies doivent absolument être cuites avant consommation. Les feuilles, racines et baies crues sont toxiques.", danger: "Baies crues et feuilles TOXIQUES — toujours cuire les baies avant consommation" },
    { nom: "Chicouté", nom_latin: "Rubus chamaemorus", emoji: "🟡", comestible: "oui", saison: [7, 8], parties: "Fruits", habitat: "Tourbières et marais du nord du Québec", description: "La chicouté, aussi appelée plaquebière, est un fruit nordique très rare et précieux. Orange à maturité, elle a un goût acidulé unique. Elle pousse uniquement dans les tourbières du nord.", conseil: "Fruit très rare et fragile. Le consommer rapidement après la cueillette.", danger: null },
    { nom: "Cèpe de Bordeaux", nom_latin: "Boletus edulis", emoji: "🍄", comestible: "oui", saison: [8, 9, 10], parties: "Chapeau et pied", habitat: "Forêts de conifères et mixtes", description: "Le cèpe est le roi des champignons. Son chapeau brun et son pied trapu le rendent reconnaissable. Excellent frais, séché ou en poudre, il parfume tous les plats.", conseil: "Vérifier que les pores sous le chapeau sont blancs à jaunes. Ne jamais bleuir à la coupe.", danger: "Bolet satan (Boletus satanas) — toxique, pores rouges et bleuissement à la coupe" },
  ...
  return (
    <div>
      <h2 style={{ color: "#2D6A4F" }}>📚 Encyclopédie des plantes</h2>

  <div style={{ background: "white", borderRadius: "16px", padding: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)", marginBottom: "20px" }}>
  <input
    type="text"
    value={recherche}
    onChange={e => { setRecherche(e.target.value); setPlanteSelectionnee(null) }}
    placeholder="🔍 Rechercher dans la liste... ex: chanterelle, pissenlit"
  ...
  />
  <p style={{ color: "#888", fontSize: "13px", margin: "0 0 10px" }}>Ou recherche n'importe quelle plante avec l'IA 🤖</p>
  <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
    <input
      type="text"
      value={rechercheIA}
      ...
      placeholder="Ex: lilas, muguet, ortie..."
    ...
    <button onClick={rechercherIA} disabled={chargementIA} style={{ ...btnPrincipal, background: "#52B788" }}>
      {chargementIA ? "Recherche..." : "🤖 Rechercher"}
    </button>
  </div>
          ...
          {plantesFiltrees.map((plante, i) => (
            <div key={i} onClick={() => setPlanteSelectionnee(plante)}
              ...
              <div style={{ fontSize: "11px" }}>
                {plante.comestible === "oui" ? <span style={{ color: "#27ae60" }}>✅ Comestible</span> :
                  plante.comestible === "avec prudence" ? <span style={{ color: "#f39c12" }}>⚠️ Avec prudence</span> :
                    <span style={{ color: "#e74c3c" }}>❌ Toxique</span>}
              </div>
              {plante.saison.includes(moisActuel) && (
                <div style={{ marginTop: "5px", fontSize: "11px", color: "#52B788", fontWeight: "bold" }}>🌿 En saison !</div>
              )}
            </div>
          ))}
        </div>
        ...
        <div style={{ background: "white", borderRadius: "16px", padding: "30px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "15px", marginBottom: "20px" }}>
            ...
            <div>
              ...
              <div style={{ marginTop: "6px" }}>
                {planteSelectionnee.comestible === "oui" ? <span style={{ background: "#27ae60", color: "white", borderRadius: "20px", padding: "3px 10px", fontSize: "13px" }}>✅ Comestible</span> :
                  planteSelectionnee.comestible === "avec prudence" ? <span style={{ background: "#f39c12", color: "white", borderRadius: "20px", padding: "3px 10px", fontSize: "13px" }}>⚠️ Avec prudence</span> :
                    <span style={{ background: "#e74c3c", color: "white", borderRadius: "20px", padding: "3px 10px", fontSize: "13px" }}>❌ Toxique</span>}
                {planteSelectionnee.saison.includes(moisActuel) && (
                  <span style={{ background: "#52B788", color: "white", borderRadius: "20px", padding: "3px 10px", fontSize: "13px", marginLeft: "8px" }}>🌿 En saison !</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "20px" }}>
            ...
            <div style={{ background: "#f9f9f9", borderRadius: "10px", padding: "12px", gridColumn: "1 / -1" }}>
              <strong style={{ color: "#2D6A4F" }}>📍 Habitat</strong>
          ...
          <div style={{ background: "#e8f5e9", borderRadius: "10px", padding: "15px", marginBottom: "15px" }}>
            <strong style={{ color: "#2D6A4F" }}>💡 Conseil de cueillette</strong>
          ...
          {planteSelectionnee.danger && (
            <div style={{ background: "#ffe0e0", borderRadius: "10px", padding: "15px" }}>
              <strong style={{ color: "#c0392b" }}>⚠️ Attention — Sosie dangereux</strong>
